...
#define CONV_FILTER_R 1
const unsigned char CONV_FILTER[(CONV_FILTER_R * 2 + 1) * (CONV_FILTER_R * 2 + 1)] = {
    1, 2, 1,
    2, 3, 2,
    1, 2, 1
};
    ...
    for (int fOffX = -CONV_FILTER_R; fOffX < CONV_FILTER_R + 1; ++fOffX) {
     for (int fOffY = -CONV_FILTER_R; fOffY < CONV_FILTER_R + 1; ++fOffY) {
         ...
         if (offInCol >= 0 && offInCol < w && offInRow >= 0 && offInRow < h) {
            unsigned char fVal = CONV_FILTER[(fOffY + CONV_FILTER_R)  * (CONV_FILTER_R * 2 + 1) + fOffX];
