...
__global__ void convolutionKernel(unsigned char *in, unsigned char *out, int w, int h) {
    ...
    // Declare shared memory to load the input values. It has to have the size of the input tiles.
    __shared__ float blockSharedEls[(TILE_SIZE + CONV_FILTER_R * 2) * (TILE_SIZE + CONV_FILTER_R * 2)];

    // Load the value to shared memory. 0 if out of bounds.
    if ((col < 0 || col >= w) || (row < 0 || row >= h)) {
        blockSharedEls[threadIdx.y * inTileSize + threadIdx.x] = 0.0;
    } else {
        blockSharedEls[threadIdx.y * inTileSize + threadIdx.x] = (float)(in[row * w + col]);
    }
    ...
    for (int convOffY = 0; convOffY < CONV_FILTER_R * 2 + 1; ++convOffY) {
        for (int convOffX = 0; convOffX < CONV_FILTER_R * 2 + 1; ++convOffX) {
            float inVal = blockSharedEls[(tileRow + convOffX) * inTileSize + (tileCol + convOffX)];
            float filterVal = (float)d_CONV_FILTER[convOffY * (CONV_FILTER_R * 2 + 1) + convOffX];
            acc += inVal * filterVal;
        }
    }
    acc /= 65.0;
