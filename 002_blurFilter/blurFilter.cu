...
__global__ void blurFilterKernel(unsigned char *inImage,
                                 unsigned char *outImage, int cols, int rows,
                                 int filterSize) {
  ...
  if (col < cols && row < rows) {
    ...
    for (int blurRowOffset = -filterSize; blurRowOffset <= filterSize;
         ++blurRowOffset) {
      for (int blurColOffset = -filterSize; blurColOffset <= filterSize;
           ++blurColOffset) {
...
void blurFilter(unsigned char *h_inImage, unsigned char *h_outImage, int cols,
  ...
  blurFilterKernel<<<dimGrid, dimBlock>>>(d_inImage, d_outImage, cols, rows,
                                          filterSize);
...
// ChatGPT generated function to load the image, did not want to spend the time
// since my focus is in CUDA
void loadInImage(const char *filename, unsigned char **inImage, int *cols,
                 int *rows) {
...
void storeOutImage(const char *filename, unsigned char *imOut, int cols,
                   int rows) {
